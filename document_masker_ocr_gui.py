...
class MaskerApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        mode_card = ttk.LabelFrame(controls, text="처리 모드")
        mode_card.grid(row=0, column=1, sticky="nsew", padx=(0, 10))
        ...
        llm_card = ttk.LabelFrame(controls, text="보조 판정 / 실행")
        llm_card.grid(row=0, column=2, sticky="nsew")
        ttk.Checkbutton(llm_card, text="로컬 LLM 재판정(애매 케이스)", variable=self.var_local_llm_refine).grid(row=0, column=0, columnspan=2, sticky="w", padx=12, pady=(12, 8))
        ttk.Label(llm_card, text="LLM 명령").grid(row=1, column=0, sticky="w", padx=12)
        ttk.Entry(llm_card, textvariable=self.var_local_llm_cmd).grid(row=2, column=0, columnspan=2, sticky="ew", padx=12)
        ttk.Label(llm_card, text="모델 경로(GGUF)").grid(row=3, column=0, sticky="w", padx=12, pady=(10, 0))
        ttk.Entry(llm_card, textvariable=self.var_local_llm_model).grid(row=4, column=0, columnspan=2, sticky="ew", padx=12)
        ttk.Label(llm_card, text="최대 호출").grid(row=5, column=0, sticky="w", padx=12, pady=(10, 0))
        ttk.Entry(llm_card, textvariable=self.var_local_llm_max_calls, width=8).grid(row=6, column=0, sticky="w", padx=12)
        self.btn_run = ttk.Button(llm_card, text="마스킹 실행", style="Accent.TButton", command=self.run_masking)
        self.btn_run.grid(row=7, column=0, sticky="ew", padx=12, pady=(18, 8))
        ttk.Button(llm_card, text="중지 요청", command=self.request_stop).grid(row=7, column=1, sticky="ew", padx=(0, 12), pady=(18, 8))
        ttk.Button(llm_card, text="창 비우기", command=self.clear_texts).grid(row=8, column=0, columnspan=2, sticky="ew", padx=12, pady=(0, 12))
        self.progress = ttk.Progressbar(llm_card, mode="determinate", length=260)
        self.progress.grid(row=9, column=0, sticky="ew", padx=12, pady=(0, 10))
        self.lbl_progress = ttk.Label(llm_card, text="0/0", style="Muted.TLabel")
        self.lbl_progress.grid(row=9, column=1, sticky="e", padx=(0, 12), pady=(0, 10))
